CREATE Procedure "SP_ITN_purchaseRequest"(IN DocKey INT)
AS
BEGIN
SELECT 
	T0."DocEntry"
   ...
   ,T0."U_CCODE" AS "VendorCode"
	,T0."ReqName" 
	,T13."Building" AS "BillingAddress"
	,(select distinct "TaxId4" from CRD7 TT where TT."CardCode"=T0."U_CCODE" and "TaxId4" is not null) AS "PartyPAN"
	,T16."Building" AS "ShippingAddress"
...
LEFT JOIN OCRD T11 ON T11."CardCode" = T0."U_CCODE"
--INNER JOIN CRD7 T12 ON T12."CardCode" = T0."U_CCODE"
--AND T12."AddrType" IN ('B', 'S')
LEFT JOIN CRD1 T13 ON T13."CardCode" = T0."U_CCODE"
	AND T13."AdresType" = 'B'
LEFT JOIN OUSR ON T0."UserSign" = OUSR."USERID"
INNER JOIN OADM ON 1 = 1
INNER JOIN OITB T15 ON T3."ItmsGrpCod" = T15."ItmsGrpCod"
LEFT JOIN CRD1 T16 ON T16."CardCode" = T0."U_CCODE"
	AND T16."AdresType" = 'S'
