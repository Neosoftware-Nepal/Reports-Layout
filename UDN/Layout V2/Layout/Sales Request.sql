...
BEGIN
	SELECT   T1."DocEntry"
		...
		,T8."ItmsGrpNam" as "Business Unit"
		...
		,0 as "QTY(PCS)"
		,ifnull(T2."U_ITNPRODISPR", 0) as "Promo Discount Percent"
		,ifnull(T2."U_ITNPRODISAM", 0) as "Promo Discount Amount"
		,IFNULL((SELECT sum(QUT4."BaseSum") FROM QUT4 WHERE T1."DocEntry" = QUT4."DocEntry" AND QUT4."LineNum" = T2."LineNum" AND QUT4."staType" = 1), 0) AS "TaxableAmount"
			
	
	FROM OQUT T1
	INNER JOIN QUT1 T2 ON T1."DocEntry" = T2."DocEntry"
	INNER JOIN OITM OT ON T2."ItemCode" = OT."ItemCode"
	INNER JOIN OITB T8 ON OT."ItmsGrpCod" = T8."ItmsGrpCod"
	...
	LEFT JOIN OUSR ON T1."UserSign" = OUSR."USERID"
