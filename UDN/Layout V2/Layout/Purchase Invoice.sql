...
SELECT distinct		T0."DocEntry"
		...
		|| '' || IFNULL(T7."EndStr",'') as "Invoice Number"
		...
		,T4."ItmsGrpNam" as "Business Unit"
		...
		,T1."WtCalced" 
		,T1."Quantity" as "A/P Qantity"
		,0 as "PCS"
		,T1."unitMsr" as "A/P Unit"
		,T1."Rate" as "Rate(PCS)"
		...
		,(T1."Quantity" * T3."NumInBuy") As "Total(PCS)"
		...
		,IFNULL((SELECT PCH4."TaxSum" FROM PCH4 WHERE T0."DocEntry" = PCH4."DocEntry" AND PCH4."LineNum" = T1."LineNum" AND PCH4."staType" = 1), 0) AS "LineVatAmt"
		,IFNULL((SELECT PCH4."TaxSum" FROM PCH4 WHERE T0."DocEntry" = PCH4."DocEntry" AND PCH4."LineNum" = T1."LineNum" AND PCH4."staType" = 7), 0) AS "LineExciseAmount"
		,IFNULL((SELECT sum(PCH4."BaseSum") FROM PCH4 WHERE T0."DocEntry" = PCh4."DocEntry" AND PCH4."LineNum" = T1."LineNum" AND PCH4."staType" = 1), 0) AS "TaxableAmount"
