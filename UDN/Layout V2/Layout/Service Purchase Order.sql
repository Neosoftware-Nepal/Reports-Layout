ALTER Procedure "SP_ITN_ServicePurchaseOrder"(IN DocKey INT)
AS
BEGIN
SELECT T0."DocEntry" 
		,(IFNULL(T4."BeginStr", '') 
	 ||''|| CAST(T0."DocNum" AS NVARCHAR) 
	 ||''|| IFNULL(CAST(T4."EndStr" AS nvarchar), '')) AS "PurOrdNo"
	,T0."DocDate" AS "PurOrdDt"
     ,ITN_NEPALI_FMT_DATE (T0."U_ITN_NPDate") AS "Miti" 
	--,CONCAT(SUBSTRING(T0."U_ITN_NPDate",0,4),'/',SUBSTR;ING(T0."U_ITN_NPDate",5,2),'/',SUBSTRING(T0."U_ITN_NPDate",7,2)) AS Miti 
...
Left JOIN  OSAC OS ON T1."SacEntry" = OS."AbsEntry"
