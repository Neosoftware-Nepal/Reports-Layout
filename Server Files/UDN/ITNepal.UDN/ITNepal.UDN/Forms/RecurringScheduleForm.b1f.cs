...
{
    [FormAttribute("Procons.AlGhanim.Xerox.Forms.RecurringScheduleForm", "Forms/RecurringScheduleForm.b1f")]
        ...
        public void LoadData(string query)
        {
            ...
            try
            {
                if (rsService.RecordCount > 0)
                {
                    ...
                    else if (BillTerms == "12")
                    {
                        oDBs_Head.SetValue("U_RBillTerms", oDBs_Head.Offset, "Monthly");
                    }
                    else if (BillTerms == "2")
                    {
                        oDBs_Head.SetValue("U_RBillTerms", oDBs_Head.Offset, "Semi-Annual");
                    }
                    else if  (BillTerms == "1")
                    {
                        oDBs_Head.SetValue("U_RBillTerms", oDBs_Head.Offset, "Yearly");
                    }
                    ...
                    if (iBillTerms == 4)
                    {
                        ...
                        dDayValue = dBillvalue / 90;
                        dMonthValue = dBillvalue / 3;
                    }
                    else if (iBillTerms == 12)
                    {
                        dDayValue = dBillvalue / 30;
                        dMonthValue = dBillvalue / 1;
                    }
                    else if (iBillTerms == 2)
                    {
                        dDayValue = dBillvalue / 180;
                        dMonthValue = dBillvalue / 6;
                    }
                    else if (iBillTerms == 1)
                    {
                        dDayValue = dBillvalue / 360;
                    ...
                    for (int iDay = 0; iDay <= datespan; iDay++)
                    {
                        ...
                        //HANA
                        string Query = "SELECT MONTHNAME(ADD_MONTHS (TO_DATE ('" + StartDates + "', 'YYYYMMDD')," + iDay + ")) \"Date\",YEAR (TO_DATE ('" + StartDates + "', 'YYYYMMDD')," + iDay + ") \"Years\" FROM DUMMY";
