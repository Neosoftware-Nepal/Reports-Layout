...
BEGIN
	SELECT T1."DocEntry"
	    ...
	    || '' || IFNULL(CAST(NM."EndStr" AS NVARCHAR(20)), '') AS "Rno"
	
	/*SELECT 	IFNULL(NM."BeginStr", '') 
	    || '' || CAST(T1."DocNum" AS CHAR(20)) 
	    || '' || IFNULL(CAST(NM."EndStr" AS CHAR(20)), '') AS "Rno" FROM NNM1 NM,ORPC T1
	    WHERE  NM."Series" = T1."Series"*/
		...
		---,CASE IFNULL(CAST(T12."TransCat" AS NVARCHAR(10)), '')
			---WHEN ''
			---	THEN ''
			---ELSE 'SALE AGAINST ' + CAST(T12."TransCat" AS NVARCHAR(10))
			---END AS "FormType"
		,T12."FormNo"
