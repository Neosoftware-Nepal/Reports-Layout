CREATE Procedure [dbo].[SP_ITN_HC_Service_purchaseOrder](@DocKey INT)
AS
BEGIN
SELECT T0."DocEntry"
	,CONCAT(ISNULL(T4."BeginStr", '') 
	 , CAST(T0."DocNum" AS NVARCHAR) 
	 , ISNULL(CAST(T4."EndStr" AS nvarchar), '')) AS "PurOrdNo"
	...
	,T11."CardName" 
	,ISNULL(T1.Dscription,'')AS "ITEM_DESCRIPTION"
	,T1.AcctCode
...
INNER JOIN  OSAC OS ON T1.SacEntry = OS.AbsEntry
WHERE T0."DocType" = 'S'
