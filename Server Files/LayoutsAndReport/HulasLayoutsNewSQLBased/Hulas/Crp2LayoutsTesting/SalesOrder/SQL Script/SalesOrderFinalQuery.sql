...
CREATE PROCEDURE [dbo].[SP_ITN_HC_salesOrder] (@DocKey INT)
AS
  BEGIN
 ...
 SELECT T0."DocEntry"
	...
	GROUP BY T0."DocEntry"
			,CONCAT(ISNULL(T20."BeginStr", '')
			, CAST(T0."DocNum" AS CHAR(20)) 
			 , ISNULL(CAST(T20."EndStr" AS CHAR(20)), ''))
			,T0."CardCode"
			,T0."CardName"
			,T0."Address2"
			,T0."Address" 			
			...
			,OADM."Country"
			,T0."OwnerCode"
			,ISNULL(CAST(AD."Building" AS VARCHAR), '')
			,ISNULL(ST."Name", '')
			,ISNULL(AD."City", '')
			,ISNULL(AD."ZipCode", '')
			,ISNULL(AD."Country", '')
			,T17."Name"
			,ISNULL(CAST(AD1."Street" AS VARCHAR), '')
			,ISNULL(AD1."City", '') 
			,ISNULL(ST1."Name", '') 
			,ISNULL(AD1."ZipCode", '')
			,ISNULL(AD1."Country", '') 
