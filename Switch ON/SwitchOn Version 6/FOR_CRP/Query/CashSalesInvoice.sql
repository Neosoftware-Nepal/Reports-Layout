ALTER PROCEDURE "SP_ITN_CashSalesInvoice" (IN DocKey INT)
AS
BEGIN
SELECT /*  Head Details   */
		T0."DocEntry" 
		...
		,IFNULL(T21."BeginStr", '') 
		|| '' || CAST(ODLN."DocNum" AS CHAR(20))|| '' || IFNULL(T20."EndStr", '')  AS "OrderNo"
		,ODLN."DocDate" AS "OrderDT"
		,ITN_NEPALI_FMT_DATE(ODLN."U_ITN_NPDate")AS "OrdrMiti"
		--,SUBSTRING(ODLN."U_ITN_NPDate", 0, 4) || '/' || SUBSTRING(ODLN."U_ITN_NPDate", 5, 2) || '/' || SUBSTRING(ODLN."U_ITN_NPDate", 7, 2) AS "OrdrNpDt"	
		,IFNULL(T20."BeginStr", '') || '' || CAST(T0."DocNum" AS CHAR(20))  || '' || IFNULL(T20."EndStr", '') AS "InvNo"
		...
		,IFNULL(M."Address", '')AS "CustAddress"
		...
		,T16."Phone1"
		,T16."Phone2"
		,T16."Cellular" AS "CustNumber"
		,IFNULL(M."E_MailL", '')  AS "CustContEmail"
		
		,T16."Balance"
		,L."Location"
		
		,ITN_NEPALI_FMT_DATE(T0."U_ITN_NPDate")"Miti"
...
LEFT JOIN OEXD I28 ON I27."ExpnsCode" = I28."ExpnsCode"
	AND I28."ExpnsCode" = 8 
	WHERE T0."CardCode" = 'CASH' and T0."DocEntry" = :DocKey;
END